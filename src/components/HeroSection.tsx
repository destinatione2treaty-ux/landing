...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#1E2F47]">
      {/* Imagen de fondo del mapa - MÁS VISIBLE */}
      <div 
      ...
      {/* Overlay gradient - MÁS SUAVE para ver el mapa */}
      <div className="absolute inset-0 bg-gradient-to-b from-[#1E2F47]/70 via-[#1E2F47]/50 to-[#1E2F47]/70"></div>
      ...
      <div className="relative z-10 container mx-auto px-6 py-20 text-center">
        ...
        {/* Título - MÁS COMPACTO Y MÁS GRUESO */}
        <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl font-black mb-6 leading-[0.95] font-headline tracking-tight">
        ...
        <p className="text-xl md:text-2xl text-gray-200 mb-4 max-w-3xl mx-auto leading-relaxed font-medium">
          {t.hero.subtitle}
        </p>
        ...
        <Button 
          ...
          size="lg"
          className="w-full sm:w-auto h-auto py-5 px-10 text-base sm:text-lg font-black uppercase tracking-wide whitespace-normal text-center bg-primary hover:bg-primary/90 text-white shadow-2xl hover:shadow-primary/50 transition-all duration-300 hover:scale-105 rounded-lg"
        >
          <span className="text-center leading-tight">
            {t.hero.ctaButton}
          </span>
          <ArrowRight className="ml-2 h-6 w-6 group-hover:translate-x-1 transition-transform flex-shrink-0" />
        </Button>
        ...
        <div className="mt-8 flex items-center justify-center gap-2">
          <div className="flex">
            {[1, 2, 3, 4, 5].map((star) => (
              <Star key={star} className="h-5 w-5 fill-[#F2A307] text-[#F2A307]" />
            ))}
          </div>
        ...
        <div className="mt-12 grid grid-cols-1 md:grid-cols-3 gap-6 max-w-3xl mx-auto">
          <div className="flex flex-col items-center gap-2 p-5 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-4xl font-black text-primary">{t.hero.badge1}</span>
            <span className="text-sm text-gray-200 text-center font-semibold">{t.hero.badge1Text}</span>
          </div>
          <div className="flex flex-col items-center gap-2 p-5 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-4xl font-black text-primary">{t.hero.badge2}</span>
            <span className="text-sm text-gray-200 text-center font-semibold">{t.hero.badge2Text}</span>
          </div>
          <div className="flex flex-col items-center gap-2 p-5 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-4xl font-black text-primary">{t.hero.badge3}</span>
