...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-[85vh] flex items-center justify-center overflow-hidden bg-[#1E2F47]">
      {/* Imagen de fondo del mapa - MÁS ZOOMEADO */}
      <div 
      ...
      <div className="relative z-10 container mx-auto px-6 py-12 text-center max-w-5xl">
        ...
        {/* Título - COMPACTO */}
        <h1 className="text-4xl sm:text-5xl md:text-6xl font-black mb-5 leading-[0.9] font-headline tracking-tight">
          <span className="text-white block">{t.hero.title1}</span>
          <span className="text-white block">{t.hero.title2}</span>
          <span className="text-primary block">{t.hero.title3}</span>
        </h1>

        <p className="text-lg md:text-xl text-gray-200 mb-3 max-w-3xl mx-auto leading-snug font-medium">
          {t.hero.subtitle}
        </p>
        <p className="text-base text-gray-300 mb-6 max-w-2xl mx-auto font-medium">
          {t.hero.description}
        </p>

        <div className="mb-6">
          <div className="inline-flex items-baseline gap-2">
            <span className="text-gray-400 text-xl line-through font-bold">{t.hero.originalPrice}</span>
            <span className="text-5xl md:text-6xl font-black text-primary">{t.hero.currentPrice}</span>
          </div>
          <p className="text-xs text-gray-300 mt-1.5 font-medium">{t.hero.paymentNote}</p>
        </div>

        <Button 
          onClick={handleCTAClick}
          size="lg"
          className="w-full sm:w-auto h-auto py-4 px-8 text-sm sm:text-base font-black uppercase tracking-wide whitespace-normal text-center bg-primary hover:bg-primary/90 text-white shadow-2xl hover:shadow-primary/50 transition-all duration-300 hover:scale-105 rounded-lg"
        >
        ...
        <div className="mt-6 flex items-center justify-center gap-2">
          ...
          <span className="text-gray-200 text-xs ml-2 font-medium">
            {t.hero.socialProof}
          </span>
        </div>
        ...
        <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4 max-w-3xl mx-auto">
          <div className="flex flex-col items-center gap-1.5 p-3 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-2xl font-black text-primary">{t.hero.badge1}</span>
            <span className="text-xs text-gray-200 text-center font-semibold">{t.hero.badge1Text}</span>
          </div>
          <div className="flex flex-col items-center gap-1.5 p-3 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-2xl font-black text-primary">{t.hero.badge2}</span>
            <span className="text-xs text-gray-200 text-center font-semibold">{t.hero.badge2Text}</span>
          </div>
          <div className="flex flex-col items-center gap-1.5 p-3 bg-gray-800/60 rounded-lg border border-primary/30 backdrop-blur-sm">
            <span className="text-2xl font-black text-primary">{t.hero.badge3}</span>
            <span className="text-xs text-gray-200 text-center font-semibold">{t.hero.badge3Text}</span>
          </div>
        </div>
      </div>
